...
public class GameView extends JPanel implements View<JPanel> {
    ...
    public boolean placeTile() {
        final Optional<TileButton<JButton>> placedTileButton = mainComponent.getBoard().getPlacedUnlockedTile();
        if (placedTileButton.isPresent()) {
            final Pair<Integer, Integer> position = mainComponent.getBoard().getTileButtonPosition(placedTileButton.get());
